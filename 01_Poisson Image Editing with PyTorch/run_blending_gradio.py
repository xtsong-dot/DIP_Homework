...
def create_mask_from_points(points, img_h, img_w):
    ...
    if len(points) > 0:
        # 确保点的数据类型是 int32，这是 cv2.fillPoly 的要求
        points = points.astype(np.int32)
        
        # 将多边形内部填充为 255
        # 注意：cv2 期望的点格式是 [pts]，其中 pts 是 (n, 1, 2) 或 (n, 2)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    """
    根据论文实现拉普拉斯损失，支持不同尺寸的图像。
    """
    # 1. 定义拉普拉斯卷积核 (3x3)
    kernel = torch.tensor([[0, 1, 0], 
                           [1, -4, 1], 
                           [0, 1, 0]], dtype=torch.float32).to(foreground_img.device)
    kernel = kernel.view(1, 1, 3, 3).repeat(3, 1, 1, 1)

    # 2. 分别计算前景和当前融合图的拉普拉斯场
    # 使用 padding=1 保持尺寸不变
    fg_lap = torch.nn.functional.conv2d(foreground_img, kernel, groups=3, padding=1)
    blended_lap = torch.nn.functional.conv2d(blended_img, kernel, groups=3, padding=1)

    # 3. 提取掩码内部的有效像素
    # 由于前后景尺寸不同，我们通过掩码索引提取像素值，转为一维向量
    # .expand_as(fg_lap) 是为了处理 RGB 三通道
    fg_values = fg_lap[foreground_mask.expand_as(fg_lap) > 0.5]
    bg_values = blended_lap[background_mask.expand_as(blended_lap) > 0.5]

    # 4. 计算 MSE Loss
    # 如果因为用户选点或者取整导致像素数量微弱不等，进行对齐
    if fg_values.numel() != bg_values.numel():
        min_size = min(fg_values.numel(), bg_values.numel())
        fg_values = fg_values[:min_size]
        bg_values = bg_values[:min_size]

    return torch.nn.functional.mse_loss(bg_values, fg_values)
