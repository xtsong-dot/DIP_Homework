...
def default_results_dir(colmap_dir: str) -> str:
    """Default to a root-level results folder next to the code files."""
    scene_name = Path(colmap_dir).name
    return str(Path("results") / scene_name)
...
class GaussianTrainer:
    def __init__(
        ...
        self.training_log_path = Path(config.checkpoint_dir) / "training_log.csv"
        if not self.training_log_path.exists():
            with open(self.training_log_path, "w", encoding="utf-8") as f:
                f.write("epoch,avg_l1_loss\n")
    ...
    def log_epoch(self, epoch: int, avg_loss: float):
        """Append epoch-level training loss for later plotting."""
...
def parse_args():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default=None,
                      help='Directory to save checkpoints (default: results/<scene>)')
    ...
    parser.add_argument('--downsample_factor', type=int, default=8,
                      help='Image downsample factor used by the dataset')
    parser.add_argument('--maximum_pts_num', type=int, default=3000,
                      help='Maximum synthetic initialization points when COLMAP is unavailable')
    parser.add_argument('--max_views', type=int, default=None,
                      help='Optional limit on the number of training views for quick smoke tests')
    parser.add_argument('--scale_multiplier', type=float, default=1.0,
                      help='Multiplier for initial Gaussian scale; use smaller values such as 0.35 for sharper splats')
...
def main():
    ...
    checkpoint_dir = args.checkpoint_dir or default_results_dir(args.colmap_dir)
    ...
    config = TrainConfig(
        ...
        log_dir=os.path.join(checkpoint_dir, "debug_images"),
    ...
    print(f"Training on {len(dataset)} images from epoch {start_epoch} to {config.num_epochs}")
    ...
    trainer.visualize_rendering(dataset, os.path.join(config.checkpoint_dir, "debug_rendering.mp4"))
