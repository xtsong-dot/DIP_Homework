...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        means2D = torch.stack((fx * x / z + cx, fy * y / z + cy), dim=-1) # (N, 2)
        ...
        eigvals = eigvals.clamp(min=0.05, max=2e3)
        covs2D = torch.bmm(eigvecs * eigvals.unsqueeze(1), eigvecs.transpose(1, 2))
    ...
    def compute_gaussian_values(
        ...
        exponent = exponent.clamp(min=-80.0, max=0.0)
        # 3DGS alpha splatting uses the unnormalized Gaussian response; the
        # peak remains 1.0 and opacity controls the contribution.
        gaussian = torch.exp(exponent) ## (N, H, W)
    ...
    def forward(
        ...
        rendered = torch.nan_to_num(rendered, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
