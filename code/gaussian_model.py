...
class GaussianModel(nn.Module):
    def __init__(
        ...
        Args:
            ...
            scale_multiplier: Multiplies the KNN-based initial Gaussian scale.
                Values below 1.0 make splats sharper and usually less blurry.
        """
        super().__init__()
        ...
        self._init_scales(points3D_xyz, scale_multiplier)
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor, scale_multiplier: float) -> None:
        """Initialize scales based on local point density"""
        # Compute mean distance to local nearest neighbors. A small K keeps
        # splats local; large neighborhoods make the reconstruction blurry.
        K = min(8, self.n_points - 1)
        if K < 1:
            self.scales = nn.Parameter(torch.zeros((self.n_points, 3), dtype=torch.float32))
            return

        dists = torch.cdist(points3D_xyz.float(), points3D_xyz.float(), p=2)
        dists, _ = torch.topk(dists, k=K + 1, largest=False, dim=1)
        dists = dists[:, 1:]
        
        # Use log space for unconstrained optimization
        mean_dists = torch.mean(dists, dim=1, keepdim=True) * scale_multiplier
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        scales = torch.exp(self.scales.clamp(min=-6.0, max=3.0))
