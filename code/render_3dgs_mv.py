...
def default_results_dir(colmap_dir):
    scene_name = os.path.basename(os.path.normpath(colmap_dir))
    return os.path.join('results', scene_name)
...
def parse_args():
    ...
    p.add_argument('--output', type=str, default=None,
                   help='Output mp4 path (default: results/<scene>/render_mv.mp4)')
    ...
    p.add_argument('--max_views', type=int, default=None,
                   help='Optional limit on the number of views loaded by the dataset')
    p.add_argument('--scale_multiplier', type=float, default=1.0,
                   help='Multiplier for initial Gaussian scale before loading the checkpoint')
...
def main():
    ...
    output = args.output or os.path.join(default_results_dir(args.colmap_dir), 'render_mv.mp4')
